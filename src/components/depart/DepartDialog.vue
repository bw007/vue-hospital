<template>
  <Dialog v-model:open="dialogOpen">
    <DialogTrigger as-child>
      <Button>Yangi bo'lim</Button>
    </DialogTrigger>
    <DialogContent class="sm:max-w-[425px]">
      <DialogHeader>
        <DialogTitle>Yangi bo'lim</DialogTitle>
        <DialogDescription>
          Yangi bo'lim nomini kiriting va saqlash tugmasini bosing.
        </DialogDescription>
      </DialogHeader>
      ...
      <form @submit.prevent="addForm" class="space-y-4">
        <FormField v-slot="{ componentField }" name="title">
          <FormItem>
            ...
            <FormControl>
              <Input type="text" placeholder="Misol: Nevrologiya" v-bind="componentField" />
            </FormControl>
            <FormMessage />
          </FormItem>
        </FormField>
